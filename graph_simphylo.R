...
#AND MANY MORE WHOO
#for whatever reason, can't seem to plot Mean_beta_MPDabund vs SR or Mean_beta_MNTDabund vs SR, gives error messages (run 3)

#checking out the effect of branchy vs tippy phylogenies, in these graphs - only the regional plots matter
ggplot(Data_storage_total,aes(x=(Mean_MNTD_pa)/(Mean_MPD_pa),y=Mean_MPD_abund,color=Scale,group=Scale,fill=Scale,alpha=0.1))+
  geom_line(size=2)+ #plots data as lines
  facet_grid(.~Scale)+
